...
class UserTest < ActiveSupport::TestCase
  ...
  test "get context" do
  	...
  	c_today = u.get_context(:today, 0, loc)
	...
	assert_nil(c_today[:location], "Non nil location for today context")
  	c_tomorrow = u.get_context(:tomorrow, 0, loc)
  	...
  	assert_nil(c_today[:location], "Non nil location for today context")
  	c_week = u.get_context(:week, 0, loc)
  	assert_equal(Time.now.to_date+7, c_week[:date], "Incorrect date for week context")
